...
fetch(endpoint)
  ...
  .then((data) => {
    const simplify = data.find((i) => i.uuid === "1");

    const name = simplify.name;
    const description = simplify.description;
    const content = simplify.content;
    const bigImg = simplify.image;
    const date = simplify.completed_on;
